...
### Task 16: Gemini provider for Ask WSSL (owner request, 2026-09-09)

The owner asked to run the assistant on Google Gemini (`gemini-3.8-flash`). Keep the Claude implementation and add a provider switch so either can be selected by configuration.

**Architecture:** `functions/api/chat.ts` keeps its guards (Origin, `validateHistory`, KV daily cap) and the SSE wire protocol (`text` | `citation{title,url,quote}` | `done{served_by}` | `error`). Behind them a `Provider` interface with two implementations: the existing Anthropic one (moved into `functions/_lib/providers/anthropic.ts`, behavior unchanged) and a new `functions/_lib/providers/gemini.ts` using the official `@google/genai` SDK (fetch-based; works on Workers). `env.LLM_PROVIDER` (`gemini` | `anthropic`, default `gemini`) selects it; `env.GEMINI_MODEL` (default `gemini-3.8-flash`) names the model; `GEMINI_API_KEY` is a secret.

**Gemini specifics:**
- Corpus delivery: the whole corpus (same `CorpusDoc[]`, rendered as `### <title>\nURL: <url>\n\n<text>` sections) plus `SYSTEM_PROMPT` goes into an **explicit context cache** (`ai.caches.create({ model, config: { systemInstruction, contents, ttl: '3600s', displayName } })`). The cache name is stored in KV (`USAGE` namespace, key `gemini-cache:<model>:<sha256 of corpus+prompt>`, value `{ name, expiresAt }`, TTL 1h). Each request reuses it via `config.cachedContent`; on a cache-not-found/expired error the function recreates the cache once and retries. If the model rejects caching (e.g. below the minimum token count or unsupported), fall back to sending the corpus inline as the first user turn (log `gemini_cache_unavailable` once per isolate).
- Streaming: `ai.models.generateContentStream({ model, contents: history, config: { cachedContent, thinkingConfig: { thinkingBudget: 0 } or the model's default if that field is rejected, maxOutputTokens: 2048 } })`; forward `chunk.text` as `text` events.
- Citations: after the stream ends, scan the full answer for `https://www.wssl.org/...` URLs (and relative `/section/.../` links the model may emit); for each distinct URL that matches a corpus doc, emit `{ type: 'citation', title: doc.title, url, quote: '' }`. Emit `{ type: 'done', served_by: <model> }`. Errors → the existing error event (status/code/detail rules unchanged).
- Prompt: the same `SYSTEM_PROMPT`, plus one extra line for Gemini: "Always end your answer with a line `Sources:` followed by the full https://www.wssl.org URLs of the pages you used, one per line." (The widget hides nothing; that line is the user's visible source list, and the server also parses it.)
- Model check: `scripts/check-gemini-model.ts` lists models with `GEMINI_API_KEY` (`ai.models.list()`) and prints whether `GEMINI_MODEL` is present; the README tells the operator to run it once.

**Files:**
- Create: `functions/_lib/providers/types.ts` (`interface Provider { stream(history: ClientMessage[], corpus: CorpusDoc[], deps): AsyncIterable<ClientEvent> }` or an equivalent that the handler wraps into the SSE `ReadableStream`), `functions/_lib/providers/anthropic.ts` (existing logic from `functions/api/chat.ts` + `sse.ts`, unchanged behavior), `functions/_lib/providers/gemini.ts`, `functions/_lib/gemini-corpus.ts` (pure: `renderCorpus(corpus)`, `corpusHash(corpus, prompt)`, `extractCitations(text, corpus)`), `scripts/check-gemini-model.ts`, `tests/gemini-corpus.test.ts`, `tests/gemini-provider.test.ts` (fake `GoogleGenAI` with `caches.create`, `models.generateContentStream`; assert cache reuse from KV, recreate-on-expired, inline fallback, citation events, error mapping), `tests/provider-select.test.ts` (handler picks the provider from `LLM_PROVIDER`; unknown value → 503 config error).
- Modify: `functions/api/chat.ts` (provider selection; guards unchanged), `functions/_lib/sse.ts` (generic: takes an `AsyncIterable<ClientEvent>` or keeps the Anthropic-specific adapter inside the anthropic provider — implementer's call, but existing sse tests must keep passing or be moved with the code), `wrangler.toml` (`LLM_PROVIDER = "gemini"`, `GEMINI_MODEL = "gemini-3.8-flash"`), `.dev.vars.example` (`GEMINI_API_KEY=`), `README.md` (secrets list, provider switch, cost section rewritten for Gemini with a pointer to Google's pricing page and the same cold/warm reasoning), `package.json` (`@google/genai` dependency), existing tests that reference the Anthropic request shape (keep them, pointed at the anthropic provider module).

**Tests must not hit the network.** The Gemini provider takes the SDK client through an injectable factory so tests pass a fake.
...
### Task 20: Replace Decap with Sveltia CMS behind an Access-protected GitHub API proxy (owner request, 2026-09-09)

Editors are non-technical; Decap's editor is the weak point. Sveltia CMS (MIT) reads the same `config.yml` and has a far better editor and asset library, but it talks to GitHub's API from the browser. Keep the email login and the bot token by giving Sveltia a proxy as its API root.

**Architecture:**
1. `/admin` serves Sveltia (vendored: `npm i @sveltia/cms` pinned; a build step copies `node_modules/@sveltia/cms/dist/sveltia-cms.js` to `public/admin/sveltia-cms.js`, git-ignored, so no third-party CDN). `config.yml` `backend` becomes `{ name: github, repo: wssl2026/wssl-cms, branch: main, base_url: <site origin>, auth_endpoint: api/cms/auth, api_root: <site origin>/api/cms/gh }` (check Sveltia's option names in its README/types: `api_root`, `graphql_api_root` or how it derives the GraphQL URL). `local_backend`/`publish_mode` handling per Sveltia docs. Local development uses Sveltia's built-in "Work with Local Repository" (File System Access API); remove `decap-server` usage from docs.
2. `GET /api/cms/auth` (behind Access like everything under `/api/cms`): verifies the Access JWT (reuse `access-jwt.ts`), then returns the popup handshake page Sveltia expects (it uses the same `postMessage` protocol as Decap: `authorizing:github` then `authorization:github:success:{"token":"…","provider":"github"}`; confirm in Sveltia's source). The token is an opaque per-session value: an HMAC (`GITHUB_BOT_TOKEN`-independent secret; use `QUESTION_LOG_SECRET`? no — add `CMS_SESSION_SECRET` secret) over the editor email + expiry, e.g. `base64url(email|exp)` + `.` + HMAC-SHA256. It never contains the bot token. Same-origin check on `postMessage` as in the old handshake (opener check + `window.location.origin`).
3. `ALL /api/cms/gh/*`: verifies the Access JWT (primary) **and** the session token from `Authorization: token …`/`Bearer …` (must decode to the same email); forwards to `https://api.github.com/<path>` with the bot token, allow-listing: `GET/POST /graphql` (POST body must reference only `wssl2026/wssl-cms`: reject GraphQL bodies mentioning another `owner:`/`name:`), `GET /user` → synthesized `{ login: <email local part>, name: <email>, email, avatar_url: '' , id: <stable hash> }` (do not forward), `GET /user/emails` → `[{ email, primary: true, verified: true }]`, any `/repos/wssl2026/wssl-cms/...` path (GET/POST/PUT/PATCH/DELETE as Sveltia needs — read its backend source to enumerate: git/refs, git/trees, git/blobs, git/commits, contents, branches, compare…), `/rate_limit`. Everything else → 403. For `POST /repos/…/git/commits` and `PUT /repos/…/contents/…` bodies, set `author: { name: <email>, email }` and `committer: { name: 'WSSL Site Editor', email: 'site-editor@wssl.org' }`. Strip hop-by-hop headers; pass through `Accept`, `Content-Type`, `If-None-Match`; return GitHub's status, body and `Content-Type`. Body size limit 15 MB (media uploads are base64 blobs). Never log the bot token, the session token, or bodies; log `{ event: 'cms_proxy', method, path (repo-relative), status }`.
4. Remove the Decap proxy: `functions/api/cms/v1.ts`, `functions/_lib/decap-proxy.ts`, their tests, `public/admin/index.html`'s Decap script, `decap-server` mentions. Keep `github-content.ts` only if still used (probably not — delete if unused).

**Files:** create `functions/_lib/cms-session.ts` (mint/verify session tokens, tests), `functions/api/cms/auth.ts`, `functions/api/cms/gh/[[path]].ts` (Pages catch-all), `functions/_lib/github-proxy.ts` (pure: `classifyRequest(method, path, body)`, `rewriteCommitBody(body, author)`, `synthesizeUser(email)`; tests), `scripts/copy-sveltia.ts`, `tests/cms-session.test.ts`, `tests/github-proxy.test.ts`, `tests/cms-auth.test.ts`, `tests/cms-gh-handler.test.ts` (fake fetch; assert forwarded URL/headers/body rewriting; 403 for disallowed paths; JWT/session mismatch → 401). Modify `public/admin/index.html`, `public/admin/config.yml`, `package.json` (`build` runs copy-sveltia), `.gitignore` (`public/admin/sveltia-cms.js`), `wrangler.toml` (comment), `.dev.vars.example` (`CMS_SESSION_SECRET=`), `README.md` (Before-this-goes-live: `CMS_SESSION_SECRET` secret; remove Decap notes), `docs/editors.md` (Sveltia workflow: email login → editor; images via drag-drop/asset library; tables — state what Sveltia supports after checking the installed version), `tests/cms-config.test.ts` (new backend keys).

**Verification without owner credentials:** unit tests; `npm run build` produces `dist/admin/sveltia-cms.js`; `wrangler pages dev` on 8790 with the localhost dev bypass: `GET /api/cms/auth` returns the handshake HTML; `GET /api/cms/gh/user` returns the synthesized user; `GET /api/cms/gh/repos/wssl2026/wssl-cms/branches/main` without a bot token → 502 config error; a disallowed path → 403. The real end-to-end (login → edit → publish) is verified by the owner on staging.

**Commit:** `feat(cms): Sveltia CMS with an Access-protected GitHub API proxy (email login, bot-token commits)`
